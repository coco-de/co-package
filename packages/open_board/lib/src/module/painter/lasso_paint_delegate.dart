...
class LassoPaintDelegate implements PaintDelegate {
  ...
  final StrokePaintDelegate _strokeDelegate = StrokePaintDelegate(strokes: []);
  ...
  void paint(ui.Canvas canvas, ui.Size size) {
    ...
    for (final stroke in allStrokes) {
      if (stroke.ink == "lasso") {
        if (lassoStroke == null ||
            DateTime.parse(stroke.createdAt)
  ...
  void drawLassoLine(ui.Canvas canvas, Stroke stroke) {
    ...
    if (selectedStrokeIds.isNotEmpty && showLassoOverlay) {
      ...
      if (currentStrokeIndex >= 0) {
        final lassoStrokes =
            allStrokes.where((s) => s.ink == "lasso").toList();
  ...
  /// 오버레이 버튼(삭제/변형) 그리기
  void drawOverlayButton(ui.Canvas canvas, Offset center, double size,
      Color color, IconData icon) {
    final radius = size / 2;
    canvas.drawCircle(center, radius, Paint()..color = color..style = PaintingStyle.fill);
    canvas.drawCircle(center, radius, Paint()..color = Colors.white..strokeWidth = 2.0..style = PaintingStyle.stroke);
  ...
  void _drawDeleteIcon(ui.Canvas canvas, Offset center) {
    final p = Paint()..color = Colors.white..style = PaintingStyle.stroke..strokeWidth = 3.0..strokeCap = StrokeCap.round;
    const s = 12.0;
    canvas.drawLine(center + const Offset(-s / 2, -s / 2), center + const Offset(s / 2, s / 2), p);
    canvas.drawLine(center + const Offset(s / 2, -s / 2), center + const Offset(-s / 2, s / 2), p);
  }

  void _drawTransformIcon(ui.Canvas canvas, Offset center) {
    final p = Paint()..color = Colors.white..style = PaintingStyle.stroke..strokeWidth = 2.5..strokeCap = StrokeCap.round;
    const s = 10.0;
    canvas.drawLine(center + const Offset(-s / 2, s / 2), center + const Offset(s / 2, -s / 2), p);
    canvas.drawLine(center + const Offset(s / 2, -s / 2), center + const Offset(s / 2 - 4, -s / 2 + 2), p);
    canvas.drawLine(center + const Offset(s / 2, -s / 2), center + const Offset(s / 2 - 2, -s / 2 + 4), p);
  }
