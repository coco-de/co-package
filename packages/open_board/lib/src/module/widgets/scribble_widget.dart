...
final class _ScribbleWidgetState extends State<ScribbleWidget> {
  ...
  bool _shouldEnablePan() {
    ...
    if (currentTool == DrawingTool.highlighter) {
      ...
      if (currentScale > 1.01) {
        return true; // 확대된 상태에서는 pan 허용 (스크롤 가능)
      } else {
        return false; // 기본 크기에서는 pan 차단 (PageView 스와이프 허용)
      }
    }
  ...
  void _updateToolsState() {
    ...
    if (state is Drawing) {
      isInteractiveNotifier.value = false;
    } else {
      isInteractiveNotifier.value = true;
    }

    // 🔥 전역 상태에서 현재 도구 확인 (modeState 대신 전역 상태 사용)
    ...
    switch (currentInk) {
      ...
      case InkModes.shape:
        // 🔄 지우개에서 다른 도구로 전환 시 명시적으로 그리기 모드로 설정
        if (isEraserToOther) {
          // 먼저 그리기 모드로 전환
          widget.notifier.setStrokeInk();
          // 그 다음 ModeNotifier 설정은 DrawingState.applyToModeNotifier에서 처리
        } else {
          widget.notifier.setStrokeInk();
        }
        break;
  ...
  void _handlePointerMove(PointerMoveEvent event) {
    ...
    if (currentMode == InkModes.text) {
      ...
      // 다른 모드(펜, 지우개 등)에서는 텍스트가 이미 선택되어 있거나 드래그 중이면 텍스트 상호작용 허용
  ...
  void _removeSelectedStrokes(List<int> strokeIds) {
    ...
    final sortedIds = List<int>.from(strokeIds)..sort((a, b) => b.compareTo(a));
  ...
  /// 선택 영역 우선 처리 (모드에 관계없이)
  bool _handleSelectionAreaFirst(PointerDownEvent event, BuildContext context) {
  ...
  bool _handleLassoOverlayButtons(Offset position) {
    ...
    // 변형 버튼 위치 (좌하단)
    final transformButtonCenter = Offset(boundingBox.left, boundingBox.bottom);
  ...
  /// 올가미 선택 영역 내부 클릭 처리
  bool _handleLassoSelectionArea(PointerDownEvent event, BuildContext context) {
    // 🔥 scribble-tools 방식: 원본 화면 좌표 직접 사용
  ...
  bool _isPointInTextBounds(Offset point, TextDrawable textDrawable) {
    final textSpan = TextSpan(
      ...
      style: TextStyle(
        ...
        fontStyle: textDrawable.isItalic ? FontStyle.italic : FontStyle.normal,
        decoration: textDrawable.isUnderlined ? TextDecoration.underline : null,
      ),
    );
