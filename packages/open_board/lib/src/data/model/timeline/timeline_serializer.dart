...
class TimelineSerializer {
  ...
  static ScribbleTimeline _mapToTimeline(Map<String, dynamic> m) => .new(
    ...
    pageIds: (m['pageIds'] as List?)?.cast() ?? [],
    events:
        (m['events'] as List?)
    ...
    snapshots:
        (m['snapshots'] as List?)
            ...
            .toList() ??
        [],
    pageUpdatedAt: (m['pageUpdatedAt'] as Map?)?.cast() ?? {},
    contentFingerprint: m['contentFingerprint'] != null
        ? _mapToFingerprint(m['contentFingerprint'] as Map<String, dynamic>)
        : null,
  );
  ...
  static TimelineEvent _mapToEvent(Map<String, dynamic> m) => .new(
    timestamp: Int64.parseInt(m['timestamp'] as String? ?? '0'),
    event: _mapToEventData(
      m['type'] as String,
  ...
  static TimelineSnapshot _mapToSnapshot(Map<String, dynamic> m) => .new(
    offsetMicros: Int64.parseInt(m['offsetMicros'] as String? ?? '0'),
    activePageIndex: m['activePageIndex'] as int? ?? 0,
    pageStrokeCounts: (m['pageStrokeCounts'] as Map?)?.cast() ?? {},
  );
  ...
  static ContentFingerprint _mapToFingerprint(Map<String, dynamic> m) => .new(
    hash: m['hash'] as String? ?? '',
    algorithm: m['algorithm'] as String? ?? 'sha256',
    metadata: (m['metadata'] as Map?)?.cast() ?? {},
  );
}
