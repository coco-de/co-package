...
class ReplayManager {
  ...
  Future<void> start(String obtPath) async {
    ...
    for (var i = 0; i < bookController.pageCount; i++) {
      ...
      _savedStrokes[pageId] = List<Stroke>.of(scribble.strokes);
    }
  ...
  void _updateAnimation() {
    ...
    final absoluteTime = _recordingOriginMicros + _controller!.positionMicros;
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            ...
            action: SnackBarAction(label: 'Replay', onPressed: _startReplay),
          ),
        );
      }
    } catch (e) {
...
class _ReplayControls extends StatelessWidget {
  const _ReplayControls({required this.progress, required this.onStop});
