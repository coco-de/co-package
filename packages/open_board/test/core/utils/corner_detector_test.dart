  ...
  group('CornerDetector', () {
    group('detectSignificantCorners', () {
      ...
      test('직선 위의 포인트 → 코너 없음 (시작/끝만)', () {
        final points = createLinePoints(fromX: 0, fromY: 0, toX: 100, toY: 0, count: 10);
        final corners = CornerDetector.detectSignificantCorners(points);
    ...
    group('isSignificantCorner', () {
      test('직각은 중요 코너', () {
        ...
        expect(CornerDetector.isSignificantCorner(prev, curr, next, 5, 120), true);
      });
      ...
      test('완만한 곡선은 코너 아님', () {
        ...
        expect(CornerDetector.isSignificantCorner(prev, curr, next, 5, 120), false);
      });
    });
