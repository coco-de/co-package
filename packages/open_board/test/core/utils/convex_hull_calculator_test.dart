...
void main() {
  group('ConvexHullCalculator', () {
    group('calculateConvexHull', () {
      ...
      test('삼각형 포인트는 모두 포함', () {
        final points = createPoints([[0, 0], [10, 0], [5, 10]]);
        final hull = ConvexHullCalculator.calculateConvexHull(points);
      ...
      test('일직선 위의 포인트', () {
        final points = createPoints([[0, 0], [5, 0], [10, 0]]);
        final hull = ConvexHullCalculator.calculateConvexHull(points);
        expect(hull.isNotEmpty, true);
      });

      test('동일 포인트', () {
        final points = createPoints([[5, 5], [5, 5], [5, 5]]);
        final hull = ConvexHullCalculator.calculateConvexHull(points);
        expect(hull.isNotEmpty, true);
      });
    });
    ...
    group('calculateConvexHullArea', () {
      ...
      test('단위 정사각형 면적', () {
        ...
        expect(ConvexHullCalculator.calculateConvexHullArea(hull), closeTo(1, 0.01));
      });
      ...
      test('삼각형 면적 (밑변 10, 높이 5)', () {
        ...
        expect(ConvexHullCalculator.calculateConvexHullArea(hull), closeTo(25, 0.01));
      });
      ...
      test('10x10 정사각형 면적', () {
        ...
        expect(ConvexHullCalculator.calculateConvexHullArea(hull), closeTo(100, 0.01));
      });
    });
  });
}
