  ...
  group('터미널 리사이즈', () {
    ...
    void expectFits(String label, AppModel m, int w, int h) {
      ...
      for (var i = 0; i < lines.length; i++) {
        expect(visibleWidth(lines[i]), lessThanOrEqualTo(w - 1),
            reason: '$label ${w}x$h: ${i + 1}번째 줄이 '
                '${visibleWidth(lines[i])}칸 — 예산 ${w - 1}칸을 넘어 래핑된다');
      }
    }

    const sizes = <(int, int)>[
      (20, 8), (24, 10), (30, 12), (40, 15), (50, 18), (60, 20),
      (72, 22), (80, 24), (90, 26), (100, 30), (120, 40), (160, 50),
      (200, 60), (300, 100), (81, 25), (76, 23), (75, 9),
    ];
    ...
    test('gh 오류 화면도 들어간다', () {
      final failed = base().copyWith(
        error: List.generate(12, (i) => 'gh: 오류 상세 $i — 아주 긴 설명이 붙는다').join('\n'),
      );
      for (final (w, h) in sizes) {
    ...
    test('리사이즈가 테이블 높이·열 폭을 새 크기로 갱신한다', () {
      ...
      expect(narrow.table.columns.last.width,
    ...
    test('피커 갱신이 체크·커서 상태를 잃지 않는다', () {
      ...
      expect(shrunk.picker!.items.map((i) => i.label),
