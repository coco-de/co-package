...
void main() {
  group('LocalRunner.svcSubcommands', () {
    ...
    test('실행 중이면(포그라운드 ./run.sh, 서비스 미설치) 두 번째 listener를 '
        '띄우지 않고 stop만 시도', () {
    ...
    test('미실행 + 서비스 미설치(재부팅으로 등록이 사라졌거나 등록한 적 없음) '
        '→ install 후 start', () {
    ...
    test('LocalStatus.empty()는 미실행+미설치이므로 install+start', () {
      expect(LocalRunner.svcSubcommands(LocalStatus.empty('/tmp/runner')), [
        'install',
        'start',
      ]);
    });
  ...
  group('LocalRunner.listenerRunningIn', () {
    const svc =
        '99170 /Users/me/actions/action-01/bin/Runner.Listener run '
        '--startuptype service';
    const foreground =
        '92948 /Users/me/actions/action-02/bin/Runner.Listener '
        'run';
  ...
  group('LocalRunner.findDirFor', () {
    ...
    test('같은 경로에 이름이 다른 러너가 구성돼 있으면 null — 엉뚱한 러너에 '
        'svc.sh를 실행하지 않는다', () {
      final root = tmp.path;
...
''');
    ...
    Future<String?> keepAlive(String plist) async {
      final r = await Process.run(
        ...
        LocalRunner.keepAliveReadArgs(plist),
      );
    ...
    test('KeepAlive가 true로 잘못 들어가 있으면 SuccessfulExit=false로 바로잡는다', () async {
      final plist = writePlist();
    ...
    test('plist를 찾지 못했으면 실패가 아니라 안내만 남기고 서비스 기동은 계속된다', () async {
      expect(await LocalRunner.hardenPlist(null), [contains('건너뜁니다')]);
    });
  });
...
''');
    ...
    test('일부가 실패해도 나머지는 적용하고 실패한 러너를 따로 알린다', () async {
      ...
      File(
        '$broken/actions.runner.coco-de.$beta.plist',
