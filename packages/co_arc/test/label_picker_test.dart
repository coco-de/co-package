...
void main() {
  group('라벨 피커 (l)', () {
    test('스코프 전체 라벨을 올리고 대상 러너 라벨만 체크한다', () {
      ...
      expect(m.picker!.items.map((i) => i.label), [
        'flutter',
        'ios',
        'serverpod',
      ]);
    ...
    test('변경 없이 Enter를 누르면 GitHub 호출 없이 닫힌다', () {
      final (model, cmd) = _run(_model(), [
        _key('l'),
      ]).update(_named(KeyCode.enter));
      expect(cmd, isNull);
  ...
  group('새 라벨 입력 (n)', () {
    test('입력한 라벨이 체크된 채로 목록에 들어가고 커서가 그 위에 선다', () {
      ...
      expect(done.picker!.items.map((i) => i.label), [
        'android',
        'flutter',
        'ios',
        'serverpod',
  ...
  group('텍스트 입력', () {
    ...
    test('피커·일반 모드의 붙여넣기는 무시한다', () {
      ...
      expect(
        _run(_model(), [_key('l'), PasteMsg('junk')]).input.value,
        isEmpty,
      );
    });
    ...
    test('ctrl+a로 맨 앞, ctrl+e로 맨 뒤', () {
      ...
      final home = _run(typed, [
        KeyPressMsg(
          const TeaKey(code: KeyCode.rune, text: 'a', modifiers: {KeyMod.ctrl}),
        ),
      ]);
      ...
      final end = _run(home, [
        KeyPressMsg(
          const TeaKey(code: KeyCode.rune, text: 'e', modifiers: {KeyMod.ctrl}),
        ),
      ]);
    ...
    test('프롬프트를 열 때마다 입력이 비어 있다', () {
      ...
      expect(
        _run(dirty, [_named(KeyCode.escape), _key('g')]).input.value,
        isEmpty,
      );
    });
  });
