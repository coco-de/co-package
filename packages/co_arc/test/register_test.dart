...
void main() {
  ...
  group('parseRegisterInput', () {
    ...
    test('이름 + 라벨 CSV', () {
      expect(parseRegisterInput('action-02 self-hosted,macOS,flutter'), (
        'action-02',
        'self-hosted,macOS,flutter',
      ));
    });

    test('이름과 라벨 사이 공백이 여러 개여도 라벨은 trim됨', () {
      expect(parseRegisterInput('action-02   self-hosted,macOS'), (
        'action-02',
        'self-hosted,macOS',
      ));
    });
