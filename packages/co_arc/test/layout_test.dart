  ...
  group('Layout 세로 예산', () {
    ...
    test('어떤 높이에서도 프레임이 터미널을 넘지 않는다', () {
      for (var h = 7; h <= 120; h++) {
        ...
        expect(
          ...
          lessThanOrEqualTo(h),
          reason:
              '높이 $h에서 ${framedLines(l)}줄을 그린다 — 한 줄만 넘쳐도 '
    ...
    test('높이를 키우면 테이블·로그가 줄지 않는다 (단조성)', () {
      ...
      for (var h = 1; h <= 200; h++) {
        ...
        expect(
          l.tableRows,
          greaterThanOrEqualTo(prevRows),
          reason: '높이 $h에서 테이블 행이 $prevRows → ${l.tableRows}로 오히려 줄었다',
        );
        expect(
          l.logLines,
          greaterThanOrEqualTo(prevLog),
    ...
    test('폭을 키워도 테이블 열이 좁아지지 않는다 (단조성)', () {
      ...
      for (var w = 1; w <= 400; w++) {
        ...
        expect(
          l.nameColumnWidth,
          greaterThanOrEqualTo(prevName),
          reason: '폭 $w에서 NAME 열이 오히려 좁아졌다',
        );
        expect(
          l.labelColumnWidth,
          greaterThanOrEqualTo(prevLabels),
  ...
  group('Layout 가로 예산', () {
    ...
    test('열 폭 합계가 한 줄 예산을 넘지 않는다', () {
      ...
      for (var w = 45; w <= 300; w++) {
        ...
        expect(
          rowWidth(l),
          lessThanOrEqualTo(l.contentWidth),
    ...
    test('LABELS가 남는 폭을 흡수한다', () {
      expect(
        Layout.forTerminal(200, 40).labelColumnWidth,
    ...
    test('NAME은 10칸 아래로 내려가지 않는다', () {
      for (var w = 20; w <= 60; w++) {
        expect(
          Layout.forTerminal(w, 40).nameColumnWidth,
