...
GitHub 조회는 `gh` CLI, 등록/정리는 이 패키지의 [`scripts/`](../scripts/)를 그대로 사용합니다.
...
이미 co-package를 clone해뒀다면 로컬 경로로 설치해도 됩니다:
...
모노레포 안에서 설치 없이 바로 실행하려면(개발 중 등):

```bash
cd packages/co_arc
dart pub get
dart run co_arc:coarc
```
...
TUI를 켤 때(그리고 등록/해제 스크립트가 끝날 때), 이 머신에 설치된 러너 전부의 launchd
plist를 훑어 **크래시 자동 복구(`KeepAlive: {SuccessfulExit: false}`)가 빠진 것에 자동으로
심습니다.** 이미 적용된 러너는 건드리지 않고, 바뀐 게 없으면 로그도 남기지 않습니다.

**지금 잡을 돌리고 있는 러너도 안전합니다** — 서비스를 껐다 켜지 않고 plist 파일만 고칩니다.
launchd는 plist를 로드 시점에 읽으므로 실행 중인 러너는 그대로 돌고, 심어둔 설정은 다음
로드(재부팅·로그인·`s` 재시작)부터 유효합니다.

하단 `로컬:` 줄에 노란 `KeepAlive 없음`이 보이면 자동 적용이 실패한 것입니다 (정상이라면
보일 일이 없습니다). 자세한 배경은
[self-hosted-runner.md](self-hosted-runner.md#재부팅-후에도-백그라운드로-유지하기).
